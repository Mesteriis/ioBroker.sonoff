...
function MQTTServer(adapter) {
    ...
    this.onStateChange = function (id, state) {
        ...
        if (server && state && !state.ack) {
            ...
            if (clients[channelId]) {
                // if (stateId.match(/POWER\d?/)) {   love to shoot yourself in the foot ??
                    if (clients[channelId]._map[stateId]) {
                        ...
                        setImmediate(sendState2Client, clients[channelId], 'cmnd/' + clients[channelId]._fallBackName + '/' + stateId, String(state.val));
                    } else {
	...
	// For B1
        Color:         {type: 'number', role: 'value.Color', 'min': 1,
            'max': 12, "states": "1:Red;2:Green;3:Blue;4:Orange;" +
            "5:Light Green;6:Light Blue;7:Amber;8:Cyan;9:Purple;" +
            "10:Yellow;11:Pink;12:White",                                  read: true, write: true},
        Dimmer:        {type: 'number', role: 'value.Dimmer', 'min': 0,
            'max': 100, 'def': 90,                                         read: true, write: true, unit: '%'},
        CT:            {type: 'number', role: 'value.CT', 'min': 153,
            'max': 500, 'def': 153,                                        read: true, write: true, unit: 'K'},
        Scheme:        {type: 'number', role: 'value.Scheme', 'min':0,
            'max': 4, 'def': 0, 'states': '0:Normal;1:Wakeup;2:CycleRGB,' +
            '3:CycleRBG,4:CycleRdm',                                       read: true, write: true}
